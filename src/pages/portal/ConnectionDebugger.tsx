...
export default function ConnectionDebugger() {
    ...
    return (
        <div className="min-h-screen bg-[var(--bg-main)] text-[var(--text-secondary)] p-8 font-mono">
            <div className="max-w-3xl mx-auto space-y-8">

                <div className="bg-[var(--bg-card-strong)] border border-[var(--bg-tertiary)] rounded-xl p-6">
                    <div className="flex items-center gap-3 mb-4">
                        <Shield className="text-cyan-400 w-8 h-8" />
                        <h1 className="text-2xl font-bold text-[var(--text-primary)]">Supabase Connection Debugger</h1>
                    </div>
                    <p className="mb-6">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full bg-[var(--bg-tertiary)] border border-[var(--border)] rounded p-3 text-[var(--text-primary)] focus:border-cyan-500 outline-none transition"
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                className="w-full bg-[var(--bg-tertiary)] border border-[var(--border)] rounded p-3 text-[var(--text-primary)] focus:border-cyan-500 outline-none transition h-24"
                                placeholder="eyJ..."
                            />
                        </div>
                    ...
                    ${status === 'testing' ? 'bg-[var(--bg-tertiary)] cursor-wait' :
                ...
                {/* Logs Console */}
                <div className="bg-[var(--bg-card)] border border-[var(--bg-tertiary)] rounded-xl p-4 h-96 overflow-y-auto font-mono text-sm">
                    <div className="text-[var(--text-muted)] mb-2 border-b border-[var(--bg-tertiary)] pb-2">Debug Log Output</div>
                    {logs.length === 0 && <span className="text-[var(--text-muted)] italic">Ready to test...</span>}
                    {logs.map((log, i) => (
